...
public class RequestVoteHandler implements IHandler<RequestVoteReq, RequestVoteRet> {
    ...
    public RequestVoteHandler(RaftNode node) {
        this.node = node;
     ...
     * @return
     */
    private RequestVoteRet requestVote(RequestVoteReq requestVoteReq){
        //1、如果收到的任期比当前任期小，返回false
        ...
        if(node.canBeVoteFor(requestVoteReq.getCandidateId(), currentTerm)){
            Pair<Integer, LogEntry> currentLastLog = node.lastLog();
            ...
            logger.info("I had vote for candidate {0}", node.getVoteFor());
        }
        return RequestVoteRet.reject(currentTerm);
    }
